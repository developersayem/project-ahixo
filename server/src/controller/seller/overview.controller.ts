...
export const getSellerDashboardStats = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (user.role !== "seller")
      throw new ApiError(403, "Only sellers can view dashboard stats");
...
export const getSellerRecentOrders = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!user) throw new ApiError(404, "User not found");
    if (user.role !== "seller")
      throw new ApiError(403, "Only sellers can view recent orders");
...
export const getSellerTopProducts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!user) throw new ApiError(404, "User not found");
    if (user.role !== "seller")
      throw new ApiError(403, "Only sellers can view top products");
