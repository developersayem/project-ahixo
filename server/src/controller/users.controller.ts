...
const updateProfileController = asyncHandler(async (req: Request, res: Response) => {
  ...
  // --------------------------
  // Handle avatar upload (local)
  // --------------------------
  const avatarFile = (req.files as { [fieldname: string]: Express.Multer.File[] })?.avatar?.[0];
  if (avatarFile?.path) {
  const backendUrl = process.env.BACKEND_URL || "http://localhost:5001";
  const normalizedPath = avatarFile.path.replace(/\\/g, "/"); // Windows fix
  user.avatar = `${backendUrl}/${normalizedPath}`;
}
